...
# - Roots (roots= + notifications/roots/list_changed)
...
puts '🧪 MCP 2025-06-18 Protocol Features Test'
puts '=' * 60
...
begin
  ...
  sampling_handler = lambda do |messages, _model_preferences, system_prompt, _max_tokens|
    ...
    puts "🤖 Sampling request received: #{prompt[0..50]}..."
    ...
    # Return a mock LLM response
    {
      ...
      'content' => {
        'type' => 'text',
        'text' => "This is a mock LLM response to: \"#{prompt}\". " \
  ...
  client.on_notification do |_server, method, params|
    ...
    when 'notifications/message'
      ...
      puts "📝 Log [#{params['level']}] #{params['logger']}: #{params['data']}"
  ...
  begin
    ...
    puts '    ✅ Log level set to debug'
  rescue StandardError => e
    puts "    ❌ Error setting log level: #{e.message}"
  end
  puts
  ...
  log_mutex.synchronize do
    if log_notifications.empty?
      puts '    ⚠️  No log notifications received yet (server may not be sending them)'
    else
      puts "    ✅ Received #{log_notifications.length} log notification(s):"
  ...
  begin
    ...
    puts '    ✅ Roots set successfully'
    puts '    Roots:'
    client.roots.each do |root|
      puts "       - #{root.name}: #{root.uri}"
    end
  rescue StandardError => e
    puts "    ❌ Error setting roots: #{e.message}"
  end
  puts
  ...
  log_mutex.synchronize do
    ...
    if roots_notifications.any?
      puts '    ✅ Server received roots notification:'
      roots_notifications.each do |log|
        puts "       - [#{log[:level]}] #{log[:data]}"
      end
    else
      puts '    ⚠️  No roots notification logged (server may not log this)'
    end
  end
  puts
  ...
  begin
    ...
    puts "    ✅ Received #{values.length} completions (total: #{total}, hasMore: #{has_more})"
  ...
  begin
    ...
    completion = result['completion'] || result
    values = completion['values'] || []
    puts "    ✅ Received #{values.length} completions"
    puts "    Values: #{values.join(', ')}"
  rescue StandardError => e
    puts "    ❌ Error: #{e.message}"
  end
  ...
  sampling_mutex.synchronize do
    if sampling_requests_received.empty?
      puts '    ⚠️  No sampling requests were received by our handler'
      puts '       (This might happen if the server timed out before our response)'
    else
      puts "    ✅ Sampling handler was invoked #{sampling_requests_received.length} time(s)"
  ...
  # ============================================================
  puts '=' * 60
  puts '✨ MCP 2025-06-18 Protocol Features Test Complete!'
  puts
  puts 'Summary:'

  # Logging summary
  log_count = log_mutex.synchronize { log_notifications.length }
  puts "  📝 Logging: #{log_count.positive? ? '✅' : '⚠️'} Received #{log_count} log notification(s)"

  # Roots summary
  roots_count = client.roots&.length || 0
  puts "  📁 Roots: #{roots_count.positive? ? '✅' : '⚠️'} #{roots_count} root(s) configured"

  # Completion summary
  puts '  🔤 Completion: ✅ Autocomplete requests working'

  # Sampling summary
  sampling_count = sampling_mutex.synchronize { sampling_requests_received.length }
  puts "  🤖 Sampling: #{sampling_count.positive? ? '✅' : '⚠️'} Handler invoked #{sampling_count} time(s)"
  ...
  puts
  puts '💡 Make sure the enhanced echo server is running:'
